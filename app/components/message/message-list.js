...
import { StyleSheet, Text, View, RefreshControl, TextInput, ListView, TouchableOpacity, Alert, StatusBar, Platform, AppState, DeviceEventEmitter, ActionSheetIOS, FlatList, ActivityIndicator } from 'react-native';
...
class MessageList extends React.Component {
    ...
    _goToMessageDetail = (itemSelected) => {
        ...
        if(this._isSendJob()){
            ...
            console.log('data flatlist: ', this.state.data);

            let _unreadMsgCount = 0;
            _.each(this.state.data, function(v,k){
                _unreadMsgCount += v.unreadMessageCount;
            })
            ...
            if(itemSelected.unreadMessageCount>0){
                let _notiOption = {
                    ...
                    chat: this.props.notification.chat > 0 ? _unreadMsgCount : 0,
        ...
        _.each(allChannel, function(v,k){
            ...
            if(!_.isEmpty(userPartnerProfile)){ // comment this will see the history of chat even partner leave the group
                let _tmp = {
                    id: k,
                    selected: false,
                    chat_id: userPartnerProfile ? userPartnerProfile.userId : '',
                    channelUrl: v.url,
                    name: userPartnerProfile ? userPartnerProfile.nickname : v.name,
                    photo: userPartnerProfile ? userPartnerProfile.profileUrl : v.coverUrl,
    ...
    _renderItem = ({item}) => {
        return (
            <MessageRowItem { ...item } isSendJob={this._isSendJob()} rowPress={ this._onRowPress } messageDetail={ this._goToMessageDetail } /> 
