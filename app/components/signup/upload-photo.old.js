...
import { StyleSheet, Image, ScrollView, Text, View, TextInput, TouchableOpacity, Alert, StatusBar, ActionSheetIOS, ActivityIndicator, Dimensions } from 'react-native';
...
class UploadPhoto extends Component{
        ...
        getApi(API_URL).then((_response) => {
            ...
            if(_response.code == 200){
                ...
                _.each(_.cloneDeep(_allImg),function(v,k){
                    _tmp.push({'id': k, 'uri':v.thumbnail_url_link, 'uuid': v.upload_session_key, _id: v._id, is_featured: v.is_featured});
                })
    ...
    _removePhoto = (_media, _mediaIndex) => {
        let that = this;
        let _mediaInfo = this.state.img[_mediaIndex] ? this.state.img[_mediaIndex] : null;

        console.log('_mediaInfo : ', _mediaInfo);
        console.log('this.state.videoFromApi : ', this.state.img);
    ...
    imageCropUpload = (imgData = null, _UUID = '') => {
        if(imgData){
            ...
            postMedia(url, [
                ...
                arrImg.push({'id': _id, tmpSource: tmpSource, 'uri':objPhoto.thumbnail_url_link, 'uuid': objPhoto.upload_session_key, _id: objPhoto._id, is_featured: objPhoto.is_featured});
